...
- **Qué**: build compatible con navegador, mismo formato JSON.
- **Por qué**: shape de log consistente entre API y web; seguro para cliente.
...
- **Qué**: generador de IDs aleatorios, URL-safe y diminuto.
- **Por qué**: correlación por request sin dependencia cripto.
- **Dónde**: `apps/api/src/middleware/request-id.ts`.
- **Cuándo**: al inicio de cada request HTTP; se propaga a líneas de log y al header `x-request-id`.
...
- **Cuándo**: en cada request que matchea una ruta guardada; los endpoints de auth fallan cerrado, los de lectura fallan abierto.
...
- **Qué**: SDK de AWS para S3; funciona con Cloudflare R2 vía endpoint custom.
...
- **Cuándo**: noche a las 03:00 UTC; on demand durante drills.

### `prom-client`

- **Qué**: registry de métricas formato Prometheus.
...
- **Por qué**: consistencia con el resto de la app web.
...
| 03:00 UTC diario | Scheduled job de Fly corre `backup.ts`; en caso de falla, `lastBackupStatus=failed`. |
| `pnpm run restore-drill` manual | Corre el script de drill en `gastos_restore_drill`. |
| Cron semanal (manual) | Corre restore-drill; actualiza entrada en el runbook. |
| Check de uptime cada 5 min | UptimeRobot pinguea `/healthz`; ante falla, notifica a Gmail. |
...
- Rate limit store: reemplazar `@upstash/ratelimit` por un token bucket sobre Postgres.
- Almacenamiento de objetos: `BACKUP_DSN` acepta cualquier endpoint compatible con S3.
- Email: `MailAdapter` permite pasar de Gmail a Resend o SES más adelante.
- Uptime monitor: migrar a BetterStack o un autoalojado.
