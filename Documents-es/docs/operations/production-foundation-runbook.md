...
### 9.1 Variables de entorno del smoke (no son secretos)

El step de Playwright smoke setea estas variables desde los secretos arriba en tiempo de ejecución; no son secretos separados:

- `SMOKE_API_URL` = `STAGING_PUBLIC_API_URL` — se pasa al spec de Playwright R-PF-11 mediante un `request.newContext({ baseURL })` por test. El web NO proxea `/auth/login` (NextAuth solo expone las rutas `signin/signout/session/csrf/callback/providers`), por lo que el smoke pega contra la API directamente.
- `SMOKE_WEB_URL` = `STAGING_PUBLIC_WEB_URL` — la lee el proyecto `smoke` en `apps/web/playwright.config.ts` para las aserciones de la página `/status` y `/api/status`.

Ambas variables son necesarias para que el step de smoke ejecute efectivamente. Los archivos de test se autogiardan con `test.skip(process.env["SMOKE_API_URL"] === undefined, ...)`, por lo que el `pnpm --filter web test` local (149/149) skipea limpiamente la aserción network-bound. Quienes corran el smoke en local deben exportar ambas variables antes de invocar `pnpm --filter web exec playwright test --project=smoke`.
