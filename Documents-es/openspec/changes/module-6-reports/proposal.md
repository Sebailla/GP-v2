...
El slice es **read-only en la capa de datos**: ninguna tabla nueva de Prisma, ninguna migración, ningún cambio de schema. Reports reutiliza el port existente `TransactionRepository.findManyForUser(userId, range)` vía `@features/transactions` y el `TotalsService` canónico. El código nuevo es una capa fina de composición (`TimeBucketService` + controllers de reports + page UI + BDD) sobre el seam de lectura.
...
- Page UI en `apps/web/app/[locale]/(app)/reports/page.tsx` (server component, locale-aware) consumiendo los componentes de `client/`.
- Integración de Recharts para `MonthlySummaryCard` y `PeriodComparisonPanel` (bar chart + line chart respectivamente).
...
- Otras librerías de charts (solo Recharts; no Visx, no Chart.js).
...
1. **Reutilizar `TotalsService` de `@features/transactions` vía DI** — NO duplicar la agregación. El port `ReportsRepository` delega per-category + per-user totals al `TotalsService`, después apila time-bucketing encima. Esto evita la trampa de dos implementaciones divergiendo.
...
4. **Las server routes retornan JSON pre-agregado**; el client renderiza tablas + Recharts. No hay chart library en el server. No hay generación de CSV en el client.
...
10. **Recharts se agrega como dependencia del workspace** en `apps/web/package.json` `dependencies.recharts`. Tree-shaking soportado; sin import global de CSS.
...
- **Recharts SSR / hydration** — Recharts es client-only por default; usarlo desde un server component dispara hydration mismatches. **Mitigation**: wrappear los componentes de chart en `'use client'` e importar Recharts solo en esos.
...
- **Bundle weight** — Recharts suma ~95 KB gzip a `apps/web`. **Mitigation**: importar solo los tipos de chart que usamos (`BarChart`, `LineChart`, `XAxis`, `YAxis`, `Tooltip`); tree-shaking se ocupa del resto. Trackear bundle size en la fase de apply.
...
4. ✅ Charts — Librería Recharts (elegida después de proponer SVG vs library).
