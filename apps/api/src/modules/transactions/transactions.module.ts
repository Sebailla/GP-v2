	...
	providers: [
		...
		},
		{
			...
			useFactory: (
				...
				// binding in production. The previous attempt injected
				// `PrismaFxRateRepository` here, which implements
				// `FxRateRepository` (a different port with
				// `findLatest`/`insert`) — that compile error was the bug
				// surfaced by the `pi-lens` automated check at session start.
