...
describe("AuthController (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    // the `ZodValidationPipe` (paired with the `@BodySchema` decorator
    // on the controller methods). The NestJS built-in `ValidationPipe`
    // requires `class-validator` which is intentionally NOT in our
    // dependencies; design §6.1 commits the slice to Zod-only.
    await app.init();
  });
  ...
  describe("DELETE /auth/sessions/:id (auth required)", () => {
    it("returns 401 when no Bearer token is supplied", async () => {
      await request(app.getHttpServer()).delete("/auth/sessions/sess-1").expect(401);
    });
  });
  ...
  describe("F4 cron — purgeExpiredResetTokens (slice 3 batch 6b)", () => {
    ...
    it("calls deleteExpired on the PasswordResetTokenRepository and returns the count", async () => {
      ...
      const { AuthCronService } = await import("../src/modules/auth/auth-cron.service.js");
