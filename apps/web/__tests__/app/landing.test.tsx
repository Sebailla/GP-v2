...
async function mockCookieStore(values: Record<string, string | undefined>): Promise<void> {
  cookieStore = {};
...
describe("LandingPage — slice 4 batch 2 (post-4e T3.3 deferred follow-up)", () => {
  ...
  it("does NOT render the unauthenticated placeholder when the session is present", async () => {
    ...
    await renderPage("en");
    expect(screen.queryByText(/Auth UI lands in slice 4/i)).not.toBeInTheDocument();
  });
});
