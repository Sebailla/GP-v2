...
vi.mock("@core/database", async () => {
  ...
  return {
    ...
    prisma: {
      ...
      $transaction: vi.fn(async (fn: (tx: { category: typeof category }) => unknown) =>
        fn({ category }),
      ),
    },
  };
...
describe("PrismaCategoryRepository", () => {
  ...
  describe("softDelete", () => {
    it("is a no-op when the row is missing (updateMany returns count=0; no throw)", async () => {
      ...
      const repo = new PrismaCategoryRepository();
      await expect(repo.softDelete("cat-missing", "user-1")).resolves.toBeUndefined();
    });
    ...
    it("is a no-op when the row is already soft-deleted (updateMany filters on deletedAt: null)", async () => {
      ...
      const repo = new PrismaCategoryRepository();
      await expect(repo.softDelete("cat-already-deleted", "user-1")).resolves.toBeUndefined();
    });
