...
vi.mock("@core/database", async () => {
  ...
  return {
    ...
    prisma: {
      ...
      $transaction: vi.fn(async (fn: (tx: { transaction: typeof transaction }) => unknown) =>
        fn({ transaction }),
      ),
    },
  };
...
describe("PrismaTransactionRepository", () => {
  ...
  describe("create — Decimal boundary (D-TX-6)", () => {
    it("serializes `amount` via `input.amount.toString()` (string, not Decimal instance)", async () => {
      vi.mocked(prisma.transaction.create).mockResolvedValue(fakeRow() as never);
  ...
  describe("update", () => {
    ...
    it("translates Prisma's P2034 (serialization failure) to TransactionNotFoundError (4R re-pass)", async () => {
      ...
      const repo = new PrismaTransactionRepository();
      await expect(repo.update("txn-1", "user-1", { updatedBy: "user-1" })).rejects.toBeInstanceOf(
        TransactionNotFoundError,
      );
    });
  ...
  describe("create — P2002 translation (forward-looking guard)", () => {
    it("translates a P2002 unique-constraint violation into `TransactionAlreadyExistsError`", async () => {
      ...
      const { TransactionAlreadyExistsError } =
        await import("../infrastructure/repositories/prisma-transaction.repository.js");
      const repo = new PrismaTransactionRepository();
      await expect(
        repo.create({
          amount: toDecimal("12.34"),
  ...
  describe("softDelete", () => {
    it("is a no-op when the row is missing (updateMany returns count=0; no throw)", async () => {
      ...
      const repo = new PrismaTransactionRepository();
      await expect(repo.softDelete("txn-missing", "user-1")).resolves.toBeUndefined();
    });
    ...
    it("is a no-op when the row is already soft-deleted (updateMany filters on deletedAt: null)", async () => {
      ...
      const repo = new PrismaTransactionRepository();
      await expect(repo.softDelete("txn-already-deleted", "user-1")).resolves.toBeUndefined();
    });
