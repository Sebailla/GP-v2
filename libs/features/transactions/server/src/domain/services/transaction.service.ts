...
export class TransactionService {
  ...
  async create(
    ...
    if (ctx.idempotencyKey !== undefined) {
      if (ctx.requestFingerprint === undefined) {
        ...
        throw new Error("TransactionService.create: idempotencyKey requires requestFingerprint");
      }
    ...
    const fx = await this.lookupFx(input.amount, input.currencyCode, input.reportingCurrencyCode);
  ...
  private async idempotencyOrReplay(
    ...
    { readonly kind: "miss" } | { readonly kind: "replay"; readonly transaction: Transaction }
  > {
  ...
  private transactionFromIdempotencyPayload(cached: IdempotencyKey): Transaction {
    ...
    return {
      ...
      reportingAmount:
        p["reportingAmount"] === null ? null : toDecimal(p["reportingAmount"] as string),
  ...
  async list(
    ...
    const repoFilter: Parameters<TransactionRepository["list"]>[0] = {
      ...
      ...(filter.currencyCode !== undefined ? { currencyCode: filter.currencyCode } : {}),
    };
  ...
  async softDelete(id: string, actorId: string): Promise<void> {
    ...
    const existing = await this.txRepo.findByIdForUserIncludingDeleted(id, actorId);
    if (existing === null) {
...
export class IdempotencyKeyReusedError extends Error {
  constructor(
    ...
    super(`Idempotency key "${key}" was previously used with a different payload`);
