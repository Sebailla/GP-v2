...
export class PrismaTransactionRepository implements TransactionRepository {
  ...
  async findByIdForUserIncludingDeleted(id: string, userId: string): Promise<Transaction | null> {
  ...
  async create(input: TransactionCreate, tx?: UnitOfWorkContext): Promise<Transaction> {
    try {
      const row = await ((tx?.tx as PrismaClient | undefined) ?? this.prisma).transaction.create({
        data: {
          amount: input.amount.toString(),
          currencyCode: input.currencyCode,
          kind: input.kind,
          reportingAmount: input.reportingAmount === null ? null : input.reportingAmount.toString(),
  ...
  async softDelete(id: string, userId: string, tx?: UnitOfWorkContext): Promise<void> {
