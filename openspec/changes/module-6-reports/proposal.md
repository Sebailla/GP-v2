...
The slice is **read-only at the data layer**: no new Prisma tables, no migrations, no schema changes. Reports reuse the existing `TransactionRepository.findManyForUser(userId, range)` port via `@features/transactions` and the canonical `TotalsService` aggregation. The new code is a thin composition layer (TimeBucketService + reports controllers + page UI + BDD) on top of the read seam.
...
1. **Reuse `TotalsService` from `@features/transactions` via DI** — do NOT duplicate aggregation. The `ReportsRepository` port delegates per-category + per-user totals to `TotalsService`, then layers time-bucketing on top. This avoids the trap of two implementations diverging.
